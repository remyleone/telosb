...
class LowLevel:
    ...
    def comRxFrame(self, rxNum):
        ...
        if rxFrame[1] == chr(0) and rxFrame[2] == rxFrame[3]:  # Add. header info. correct?
            ...
            if rxFrame[ord(rxFrame[2]) + 4] == chr(0xff & checksum) \
                    and rxFrame[ord(rxFrame[2]) + 5] == chr(0xff & (checksum >> 8)):  # Checksum correct?
...
def hexify(line, bytes, width=16):
    return '%04x  %s%s %s' % (
        ...
        '   ' * (width - len(bytes)),
        ('%c' * len(bytes)) % tuple(map(lambda x: (32 <= x < 127) and x or ord('.'), bytes))
    )
